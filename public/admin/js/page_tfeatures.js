(function ($) {
    ...
    $(document).on("click", ".amenities_edit", function () {
        ...
        if (amenities_icon) {
            ...
            $("#preview_icon").attr(
                "src",
                `https://via.placeholder.com/100x100?text=icon`
            );
        }
    ...
    $(document).on("click", ".amenities_delete", function () {
        if (
            confirm("Are you sure? The amenities that deleted can not restore!")
        ) {
            $(this).parent().submit();
        }
    });

    $("#btn_add_tfeatures").click(function () {
        $("#submit_add_amenities").show();
        $("#submit_edit_amenities").hide();
        $("#add_amenities_method").val("POST");
        $("#modal_add_tfeatures").modal("show");
    });
