...
class Store extends Model  implements TranslatableContract
{
    ...
    protected $fillable = [
        'user_id', 'country_id', 'city_id', 'category',  'slug',
        'address', 'lat', 'lng', 'email', 'phone_number', 'website', 'social', 'opening_hour',
    ...
    {
        return $this->hasMany(Wishlist::class, 'item_id', 'id')->where('user_id', Auth::id());
    }
    ...
    public function listByCity($city_id)
    {
        $stores = self::query()
            ->with('city')
            ->with('categories')
        ...
        if ($city_id)
            $stores->where('country_id', $city_id);
